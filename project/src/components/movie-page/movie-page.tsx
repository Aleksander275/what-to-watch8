...
import { fetchCurrentFilmAction, fetchFilmsSimilarAction, statusFilmFavorite } from '../../store/api-action';
import { store } from '../..';
...
function MoviePage (): JSX.Element {
  ...
  const [ isFavorite, setIsFavorite ] = useState(currentFilm?.isFavorite);

  const checkedStatusFilm = (filmFavoriteId: number, status: number) => {
    if (!filmFavoriteId) {return;}
    dispatch(statusFilmFavorite(filmFavoriteId, status));
